...
class AnalyticsWindow(QWidget):
    ...
    def load_data(self):
        try:
            ...
            # --- BAR GRAPH: Average Clarity per Course ---
            ...
            if course_scores:
                ...
                bars = pg.BarGraphItem(
                    x=x, height=averages, width=0.6,
                    brush=pg.mkBrush("#2E86AB"), pen=pg.mkPen("#1B4F72")
                )
                self.bar_graph.addItem(bars)
                self.bar_graph.getAxis('bottom').setTicks([list(zip(x, courses))])

            # --- LINE GRAPH: Engagement Trend Over Time ---
            self.line_graph.clear()
            
            # REFINEMENT: Reverse the data so it plots from oldest to newest
            chronological_evals = list(reversed(evaluations))
            
            engagement = [e.get("Engagement_Score", 0) for e in chronological_evals]

            if engagement:
                # Use actual session/entry numbers for the X-axis
                entries = list(range(1, len(engagement) + 1))
                self.line_graph.plot(
                    entries, engagement,
                    pen=pg.mkPen(color="#F6C85F", width=4),
                    symbol='o', symbolBrush="#F6C85F",
                )
                # Label X-axis as 'Entry 1', 'Entry 2', etc.
                self.line_graph.getAxis('bottom').setTicks(
                    [list(zip(entries, [f"E{i}" for i in entries]))]
                )
