...
class DashboardWorker(QThread):
    ...
    def run(self):
        try:
            params = {}
            if self.prof_id: params['prof_id'] = self.prof_id
...
class DashboardPage(QWidget):
    def __init__(self, main_window):
        ...
        if not pixmap.isNull():
            nav_logo.setPixmap(pixmap.scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
        ...
        self.week_label.setStyleSheet("background-color: #6d28d9; color: white; padding: 8px 16px; border-radius: 6px; font-weight: bold; font-size: 14px;")
        ...
        nav_layout.addLayout(title_container) # Swapped standalone title for combined layout!
        ...
        self.table.setHorizontalHeaderLabels(["Week", "Course", "Topic", "Clarity", "Pacing", "Understanding", "Engagement", "Hours", "Comments"])
    ...
    def fetch_current_week(self):
        if self.week_worker is not None and self.week_worker.isRunning(): return
    ...
    def handle_week_success(self, data):
        if data["status"] in ["Not Started", "Sembreak"]: self.week_label.setText(f"Status: {data['status']}")
        else: self.week_label.setText(f"Week {data['week_number']} ({data['status']})")
    ...
    def fetch_live_data(self):
        if self.dash_worker is not None and self.dash_worker.isRunning(): return
        prof_id = None
    ...
    def on_mode_changed(self):
        ...
        for ev in self.all_evaluations:
            if mode == "By Course": unique_targets.add(ev.get("Course_Code", ""))
            elif mode == "By Professor": unique_targets.add(ev.get("Professor_Name", ""))
        ...
        for ev in self.all_evaluations:
            if target in ["All Courses", "All Professors"] or \
               ...
               (mode == "By Professor" and ev.get("Professor_Name") == target):
                ...
                if week_num > 0: unique_weeks.add(f"Week {week_num}")
    ...
    def open_analytics(self):
        try:
            ...
            QMessageBox.critical(self, "File Error", "Could not find pages/analytics.py")
        except Exception as e:
